...
from application.src.models.modelsUser import (
    Cadastro,
    Links,
    Login,
    UserInformation,
...
def create_database():
    ...
    # Essas duas TABELAS devem continua tendo relação com o id , para buscar
    # as infor do usuario etc..
    cursor.execute(
...
# add column e usada para cria novas coluunas teste, para não precisar
# excluir o banco
def add_column():
    banco, cursor = my_db()
    ...
    ]  # Obtém todos os nomes das colunas
    ...
    if "followers" not in columns:
        # add followers
        cursor.execute(
            "ALTER TABLE usuarios ADD COLUMN followers INTEGER DEFAULT 0"
        )
        banco.commit()
    ...
    if "following" not in columns:
        # add following
        cursor.execute(
            "ALTER TABLE usuarios ADD COLUMN following INTEGER DEFAULT 0"
        )
        banco.commit()
    ...
    if "is_first_login" not in columns:
        cursor.execute(
            "ALTER TABLE usuarios ADD COLUMN is_first_login BOOLEAN DEFAULT 1"
        )
        banco.commit()
    else:
        pass
...
def link_of_user(link: Links, user_id: int):
    banco, cursor = my_db()
    try:
        # Atualiza os campos github, linkedin e site do usuário com o
        # ID especificado
        cursor.execute(
            """
        UPDATE usuarios
...
class User(UserMixin):
    ...
    def get(user_id):
        ...
        banco, cursor = my_db()
        cursor.execute(
            "SELECT id, name, email FROM usuarios WHERE id = ?", (user_id,)
        )
        user = cursor.fetchone()
        banco.close()
        if user:
            print(
                f"ID: {user[0]}, Username: {user[1]}, Email: {user[2]}"
