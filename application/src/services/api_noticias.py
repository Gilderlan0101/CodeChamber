...
def get_exact_count():
    """ "
     Percorre a lista de posts retornada pela função `fetch_api_data()`
      e calcula a quantidade total de posts.

    Para cada post encontrado, incrementa o contador, que será utilizado
     para associar notícias
    ou conteúdos específicos na página inicial.

    Returns:
        int: O número total de posts encontrados na lista retornada por
         `fetch_api_data()`.
    """
...
def get_top_stories(
    num_noticias=get_exact_count(),
):  # Define dinamicamente a quantidade de notícia
    api_url = os.getenv("API_NOTICIA")  #  API

    if not api_url:
        print(
            "⚠️ Por favor, configure a chave da API de notícias! Acesse"
            "https://developer.nytimes.com/ para obter sua chave."
        )
        return []
    ...
    if response.status_code == 200:
        ...
        if "results" in news_data:
            ...
            logging.error(
                "Error: No results found in the Notices API response."
            )
            return []
    else:
        logging.error(
            f"Error: in api of noctic status code. {response.status_code}"
        )
        return []
