...
@configuracao_.route(
    "/devorbit/configuracao/<usuario>", methods=["POST", "GET"]
)
def config_account(usuario):
    ...
    if request.method == "POST":
        ...
        if (
            ...
            flash(
                "Erro ao salvar links. Verifique os links e tente novamente.",
                "error",
            )
            return redirect(url_for("config.config_account", usuario=usuario))
    ...
    try:
        ...
        status = (
            "Conta Saudável" if usuario else "Sua conta está sendo verificada."
        )
        ...
        flash(
            "Erro ao carregar a página. Tente novamente mais tarde.", "error"
        )
